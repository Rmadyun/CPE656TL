...
/**
 * Class responsible for launching a Debug Implementation of the Restful Service
 * of the Train Navigation Service. This is intended to assist with integration
 * between clients of the service.
 * 
...
public class DebugService {
	 ...
	 * @throws Exception
	 *             Reports any failure to create the Train Navigation Service
	 *             instance
	 */
	private static TrainNavigationServiceInterface createTrainNavigationServiceWithFakeSwitchControllerAndRealMdu()
			throws Exception {
		...
		MotionDetectionUnit motionDetectionUnit = new MotionDetectionUnit(
				new SerialPortMduCommunicationChannel("/dev/ttyUSB0"), new MduProtocolParser());
	 ...
	 *             instance
	 */
	private static TrainNavigationServiceInterface createTrainNavigationServiceWithSwitchController() throws Exception {
		TrainNavigationServiceBuilder builder = TrainNavigationServiceBuilder.getBuilder();
		...
		try {
			trackSwitchController = new TrackSwitchController(pr3Port, TrackSwitchController.DefaultPrefix, trainNavigationDatabase);
	...
	public static void main(String[] args) throws Exception {
		...
		// Create a train navigation service instance with all simulated
		// dependencies
		TrainNavigationServiceBuilder builder = TrainNavigationServiceBuilder.getBuilder();
		...
		PositionTestCase positionTestCase = MduMeasurementGenerator.generateStraightLine(
				currentOrientation.getRadiansRotationAlongZAxis(), currentPosition, initialSpeedInMetersPerSecond,
				accelerationInMetersPerSecondSquared, numberOfSeconds, startTime, numSamplesBeforeTagEvent,
				kineticFrictionOffset);
